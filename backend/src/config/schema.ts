...
export const invoices = mysqlTable(
  'invoices',
  {
    ...
    paymentStatus: mysqlEnum('payment_status', ['PENDING', 'PAID', 'PARTIAL', 'REFUNDED'])
      .notNull()
      .default('PENDING'),
    paymentMethod: mysqlEnum('payment_method', ['CASH', 'BANK_TRANSFER', 'CREDIT', 'PROMPTPAY']),
    version: int('version').notNull().default(1),
    createdBy: varchar('created_by', { length: 36 }).notNull(),
    createdAt: timestamp('created_at').notNull().defaultNow(),
    updatedAt: timestamp('updated_at').notNull().defaultNow().onUpdateNow(),
...
export const jobs = mysqlTable(
  'jobs',
  {
    ...
    status: mysqlEnum('status', ['QUEUED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED'])
      .notNull()
      .default('QUEUED'),
...
export const jobStatusLogs = mysqlTable(
  'job_status_logs',
  {
    ...
    fromStatus: mysqlEnum('from_status', ['QUEUED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED']),
    toStatus: mysqlEnum('to_status', ['QUEUED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED']).notNull(),
